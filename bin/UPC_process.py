...
import os, sys, pvl, subprocess, datetime, pytz
import logging
import json
...
#    engine = create_engine('postgresql://upcmgr:un1pl@c0@dino.wr.usgs.gov:3309/upc_dev')
    ...
    class meta_bands(Base):
        __table__ = Table('meta_bands', metadata, autoload=True)
            ...
            for item in recipeOBJ.getProcesses():
                if status == 'error':
                    ...
                    if '2isis' in item:
                        ...
                        processOBJ.updateParameter('from_', inputfile)
                        processOBJ.updateParameter('to', outfile)
                        thmproc_odd = workarea + os.path.splitext(os.path.basename(inputfile))[0] + '.UPCoutput.raw.odd.cub'
                        thmproc_even = workarea + os.path.splitext(os.path.basename(inputfile))[0] + '.UPCoutput.raw.even.cub'
                    elif item == 'handmos':
                    ...
                    for k, v in processOBJ.getProcess().items():
                        ...
                        try:
                            ...
                            if '2isis' in item:
                                ...
                                infile_bandlist = label['IsisCube']['BandBin'][PDSinfoDICT[getMission(inputfile)]['bandbinQuery']]
...
## keyword definition stuff ##
            if status == 'success':
                ...
                Qobj = session.query(datafiles).filter(datafiles.isisid == keywordsOBJ.getKeyword('Parameters', 'IsisId')).first()
                UPCid = Qobj.upcid
#block to add band information to meta_bands 
                if type(infile_bandlist) == list:
                    index = 0
                    while index < len(infile_bandlist):
                        print 'test of filter: %s' % str(infile_bandlist[index])
                        print 'test of center: %s' % str(infile_centerlist[index])
...
#  Block to add common keywords
                testjson = json.load(open('/usgs/cdev/PDS/recipe/Keyword_Definition.json', 'r')) 
...
#block to deal with mission keywords
                for element_2 in testjson['instrument'][getMission(inputfile)]:
                    ...
                    if M_keygroup == 'Polygon':
                        ...
                        M_keyvalue = keywordsOBJ.getKeyword(M_keygroup, M_keyword)
...
# add error keyword to UPC
                ...
                if '2isis' in processError or processError == 'thmproc':
                    ...
                    testspacecraft = PDSinfoDICT[getMission(inputfile)]['UPCerrorSpacecraft']
                    testinst = PDSinfoDICT[getMission(inputfile)]['UPCerrorInstrument']
                    
                    if session.query(datafiles).filter(datafiles.edr_source == EDRsource).first() == None:
                        target_Qobj = session.query(targets).filter(targets.targetname == keywordsOBJ.getKeyword('Instrument', 'TargetName').upper()).first()
                        instrument_Qobj = session.query(instruments).filter(instruments.instrument == keywordsOBJ.getKeyword('Instrument', 'InstrumentId')).first()
                    ...
                    EQ1obj = session.query(datafiles).filter(datafiles.edr_source == EDRsource).first()
                    UPCid = EQ1obj.upcid
                    errorMSG = 'Error running ' + processError + ' on file ' + inputfile 
                    ...
                    DBinput = meta_boolean(upcid=UPCid,
                                           ...
                                           value='true')
                    session.add(DBinput)
                    DBinput = meta_geometry(upcid=UPCid,
                    ...
                    if session.query(datafiles).filter(datafiles.isisid == isisSerial).first() == None:
                        target_Qobj = session.query(targets).filter(targets.targetname == str(label['IsisCube']['Instrument']['TargetName']).upper()).first()
                        instrument_Qobj = session.query(instruments).filter(instruments.instrument == str(label['IsisCube']['Instrument']['InstrumentId'])).first()
                    ...
                    EQ2obj = session.query(datafiles).filter(datafiles.isisid == isisSerial).first()
                    UPCid = EQ2obj.upcid
                    errorMSG = 'Error running ' + processError + ' on file ' + inputfile
